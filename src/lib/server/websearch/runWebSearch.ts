...
import { WebSearchProvider, type WebSearch, type WebSearchSource } from "$lib/types/WebSearch";
...
export async function runWebSearch(
	...
	try {
		...
		if (searchProvider === WebSearchProvider.GOOGLE) {
			...
			// you.com already provides chunked & reranked results
			console.log(`here aer web serarch ${JSON.stringify(webSearch)}`)
			const joined_context = webSearch.results.map((item) => item.text ?? "").join("\n");
			console.log(`SOME DUMBY ${joined_context}`)
			webSearch.context = joined_context;
			webSearch.contextSources = webSearch.results;
		}
